...
export function PrimaryButton({ href, text, onClick, compact = false }) {
    return (
        <Link
            href={href}
            onClick={onClick}
            className={"box-border self-center text-center text-white transition-all duration-200 bg-red-700 border-4 border-red-700 outline-1 outline-red-700 hover:shadow-lg hover:outline" + (compact ? " w-40 py-1.5" : " px-8 py-3 text-2xl")}
        >
            {text}
        </Link>
    );
